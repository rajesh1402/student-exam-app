...
async function seed() {
  ...
  for (const type of examTypes) {
    for (const subject of subjectNames) {
      await db.insert(schema.exams).values({
        subjectId: db.select({ id: schema.subjects.id }).from(schema.subjects).where(schema.subjects.name.equals(subject)),
        name: `${subject} ${type}`,
        type,
  ...
  for (const subject of subjectNames) {
    const subjectId = await db.select({ id: schema.subjects.id }).from(schema.subjects).where(schema.subjects.name.equals(subject)).then(res => res[0].id);
